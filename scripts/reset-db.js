...
async function resetDb() {
  ...
  try {
    ...
    await AdminUser.deleteMany({ email: { $ne: "pranshu" } });
    console.log("✅ Purged AdminUsers (Kept pranshu)");

    const salt = await bcrypt.genSalt(10);
    
    const pranshuExists = await AdminUser.findOne({ email: "pranshu" });
    if (!pranshuExists) {
        console.warn("⚠️ Warning: 'pranshu' admin user not found in DB! Please create it manually if needed.");
    }

    const defaultUsers = [
      { name: "Chief Warden", email: "warden", password: await bcrypt.hash("warden123", salt), role: "WARDEN" },
      { name: "Boys Rector", email: "boysrector", password: await bcrypt.hash("boysrector123", salt), role: "BOYS_RECTOR" },
      { name: "Girls Rector", email: "girlsrector", password: await bcrypt.hash("girlsrector123", salt), role: "GIRLS_RECTOR" },
    ];

    for (const u of defaultUsers) {
      await AdminUser.findOneAndUpdate({ email: u.email }, u, { upsert: true });
    }
    console.log("✅ Seeded default Warden/Rector accounts");
